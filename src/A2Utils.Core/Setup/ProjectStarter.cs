...
public static class ProjectStarter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (profile.TemplateImage is null)
        ...
        ProjectManifest manifest = new()
        {
            ...
            Output = "build/program." + (profile.TemplateFileSystem == "dos33" ? "do" : "po"),
            Disk = new() { FileSystem = profile.TemplateFileSystem },
            BasicWorkspaceBytes = language == "basic" ? 1024 : 0,
            Files = [new() { Source = "main." + extension, Path = "AI.MAIN", Kind = language == "c" ? "cc65" : language }],
        ...
        ExecutionSpec execution = new()
        {
            ...
            Keys = [new(boot, (language == "basic" ? "RUN" : "BRUN") + " AI.MAIN\r")],
            Until = new(770, 90, boot + 1),
        ...
        Dictionary<string, string> files = new(StringComparer.Ordinal)
        {
            ...
            ["README.md"] = "# Apple II starter\n\n"
                + "Configure the environment profile with your local MAME 0.289, ROMs, and bootable DOS/ProDOS template. "
                + "The template must reach an Applesoft prompt; ProDOS needs BASIC.SYSTEM. No external assets are installed or bundled. "
                + "Set bootSeconds to the template's startup time. The new program name AI.MAIN must not already exist in the template.\n\n"
        ...
        try
        {
            ...
            bool needsConfiguration = environmentProfile is null || !File.Exists(profile.MamePath) || !Directory.Exists(profile.RomDirectory)
                || !File.Exists(profile.TemplateImage) || language == "c" && (!File.Exists(profile.Cc65Path) || !Directory.Exists(profile.Cc65Root));
